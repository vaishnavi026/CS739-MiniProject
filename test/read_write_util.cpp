...
int main(int argc, char **argv) {
  ...
  if(read_or_write == 1) //Write condition
  {
      ...
      put_result = kv739_put((char*)write_key.data(), (char*)write_value.data(), old_value);
      if(put_result != -1){
        std::cout << "PASS : Wrote value = " << write_value << " at key = " << write_key << " for client = " << client_number << "\n";
      }else{
      ...
      if(get_result == 0){
        if(strcmp(get_value,expected_read_value.data()) == 0){
            std::cout << "PASS : Read value = " << get_value << " matched the expected value for client = " << client_number <<  "\n";
        }else{
            std::cout << "FAIL : Read value = " << get_value << " did not match the expected value for client = " << client_number <<  "\n";
        }
      }else if(get_result == 1){
        std::cout << "FAIL : No value exists for the given key = " << read_key << " in the key value store\n";
      }else{
        std::cout << "FAIL : Error occured, the read was not succesful\n";
      }
  }
